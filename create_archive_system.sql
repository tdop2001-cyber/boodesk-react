...
CREATE TABLE IF NOT EXISTS archived_cards (
    ...
    retention_period INTEGER DEFAULT 365, -- Dias para manter arquivado
    auto_archived BOOLEAN DEFAULT false, -- Se foi arquivado automaticamente
...
CREATE TABLE IF NOT EXISTS archive_settings (
    ...
    archive_after_days INTEGER DEFAULT 30, -- Dias após conclusão para arquivar
...
CREATE TABLE IF NOT EXISTS archive_history (
    ...
    action VARCHAR(50) NOT NULL, -- 'archived', 'restored', 'deleted'
...
-- 5. Adicionar colunas de arquivamento na tabela cards
ALTER TABLE cards 
...
BEGIN
    ...
    FOR card_record IN 
        SELECT c.*, s.archive_after_days, s.default_folder_id
        FROM cards c
        LEFT JOIN archive_settings s ON c.board_id::integer = s.board_id
        WHERE c.is_archived = false 
        AND c.status = 'done'
        AND c.completed_at IS NOT NULL
        AND (
            s.auto_archive_enabled = true 
            AND c.completed_at <= NOW() - INTERVAL '1 day' * COALESCE(s.archive_after_days, 30)
        )
    LOOP
        -- Determinar pasta de destino
        default_folder_id := COALESCE(card_record.default_folder_id, 1); -- Pasta padrão
        ...
        SET 
            ...
            archive_folder_id = default_folder_id
        ...
        INSERT INTO archived_cards (
            ...
            'Arquivamento automático após ' || card_record.archive_after_days || ' dias',
            true
        );
        ...
        INSERT INTO archive_history (
            ...
            jsonb_build_object(
                'auto_archived', true,
                'archive_after_days', card_record.archive_after_days,
...
(NULL, false, 30, 4, 1) -- Configuração global padrão
ON CONFLICT DO NOTHING;
...
LEFT JOIN boards b ON c.board_id::integer = b.id
