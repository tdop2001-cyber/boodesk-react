...
const SubtaskModal: React.FC<SubtaskModalProps> = ({
  ...
  const [subtaskTags, setSubtaskTags] = useState<string[]>(subtask?.tags || []);
  ...
  const handleSave = async () => {
    ...
    try {
      if (isCreateMode && onSubmit) {
        ...
        // Modo de edição
        const success = await db.updateSubtask(subtask.id, {
          ...editedSubtask,
          tags: subtaskTags
        });
  ...
  // Função para gerenciar tags da subtask
  const handleTagsChange = async (newTags: string[]) => {
    try {
      setSubtaskTags(newTags);
      
      // Se estiver editando uma subtask existente, atualizar no banco
      if (subtask?.id && isEditing) {
        await db.updateSubtask(subtask.id, { tags: newTags });
        
        // Atualizar a subtask local
        const updatedSubtask = { ...editedSubtask, tags: newTags };
        setEditedSubtask(updatedSubtask);
        
        addToast({
          type: 'success',
          title: 'Tags atualizadas',
          message: 'Tags da subtarefa foram atualizadas com sucesso!'
        });
      }
    } catch (error) {
