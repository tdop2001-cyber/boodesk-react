-- Sistema de Arquivamento Automático Contínuo - VERSÃO CORRIGIDA
...
-- 2. Criar tabela para controle de execução (se não existir)
...
-- 6. Ativar arquivamento automático global (SEM ON CONFLICT)
...
-- 7. Ativar todas as configurações existentes
UPDATE archive_settings 
SET auto_archive_enabled = true 
...
-- 10. Mostrar monitoramento
SELECT * FROM archive_system_monitor;

-- 11. Verificar logs de execução
SELECT 
...
-- 12. Verificar configurações ativas
SELECT 
    'Configurações ativas' as status,
    id,
    board_id,
