...
-- Versão final com verificações de existência de tabelas
    ...
    LOOP
        ...
        FOR card_record IN 
            ...
            AND (
                ...
                OR c.board_id = settings_record.board_id::text
            )
        LOOP
            -- Arquivar o card
            UPDATE cards 
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'lists') THEN
        ...
        LEFT JOIN lists l ON c.list_name = l.name
        WHERE c.is_archived = true';
    ELSE
        -- Criar view sem JOIN para lists
        EXECUTE '
        CREATE OR REPLACE VIEW archived_cards_view AS
        SELECT 
            c.id,
...
-- 15. Verificar se a view foi criada
SELECT 
