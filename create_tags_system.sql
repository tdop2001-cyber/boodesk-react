-- Sistema de Tags para Cards e Subtasks
-- Execute este SQL no Editor SQL do Supabase

-- 1. Adicionar coluna tags na tabela cards se não existir
ALTER TABLE cards ADD COLUMN IF NOT EXISTS tags TEXT[] DEFAULT '{}';

-- 2. Adicionar coluna tags na tabela subtasks se não existir  
ALTER TABLE subtasks ADD COLUMN IF NOT EXISTS tags TEXT[] DEFAULT '{}';

-- 3. Criar tabela para tags personalizadas
CREATE TABLE IF NOT EXISTS custom_tags (
    id SERIAL PRIMARY KEY,
    name VARCHAR(50) NOT NULL,
    color VARCHAR(7) DEFAULT '#6B7280',
    type VARCHAR(20) DEFAULT 'category', -- 'category', 'priority', 'status', 'custom'
    description TEXT,
...
-- 4. Criar tabela para categorias de cards
...
-- 7. Criar função para buscar tags disponíveis
...
-- 8. Criar função para buscar categorias disponíveis
...
-- 9. Criar função para adicionar tag a um card
...
-- 10. Criar função para remover tag de um card
...
-- 11. Criar função para adicionar tag a uma subtask
...
-- 12. Criar função para remover tag de uma subtask
...
-- 13. Criar função para buscar cards por tag
...
-- 14. Criar função para buscar subtasks por tag
...
-- 15. Atualizar registros existentes para ter arrays de tags vazios
UPDATE cards SET tags = '{}' WHERE tags IS NULL;
UPDATE subtasks SET tags = '{}' WHERE tags IS NULL;

-- 16. Criar índices para melhor performance
...
-- Mensagem de sucesso
SELECT 'Sistema de tags criado com sucesso!' as message;
